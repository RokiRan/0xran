...
结论：200~400 token 是登录页级别的宣传话术，复杂页面该大还是大；但**压缩效率和生成速度是真实的**，内核级定制不是空话。
...
One more thing：这篇评测从素材抓取、功能实测到截图产出，全程是 ego lite 自己跑的。能独立完成对自己的评测，大概是对一个 Agent 浏览器最硬核的验证方式了。
